...
{: note}
IOPS scaling is only enabled when disk size is at least 690 GB.
...
2. Select the **Compute & storage** tab.
3. Select **Edit** under the **Compute & storage resources**.
4. Select the desired **Units** for storage, IOPS, and throughput to make changes.
5. Click **Save**.
6. Select **Confirm** if you are satisfied with the changes.
...
{: important}
A maximum of 3 scale attempts are allowed within a 24 hour period.
